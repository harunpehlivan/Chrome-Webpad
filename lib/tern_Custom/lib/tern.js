...
(function (mod) {
    ...
    var Server = exports.Server = function (options) {
        ...
        this.resolvePos = function (file, pos, tolerant) { return resolvePos(file, pos, tolerant); }
    ...
    function findCompletions(srv, query, file) {
        ...
        query.caseInsensitive = true;//MY CAHNGE-- always want this!
        ...
        function gather(prop, obj, depth) {
            ...
            //if (query.filter !== false && word && (query.caseInsensitive ? prop.toLowerCase() : prop).indexOf(word) != 0) {//default: starts with
        	//if (query.filter !== false && word && (query.caseInsensitive ? prop.toLowerCase() : prop).indexOf(word) === -1) {//use contains filtering --MYCHANGE
        	// if (query.filter !== false && word && !(query.caseInsensitive ? prop.toLowerCase() : prop).smartContains(word)) { return; } //works but kind of sucks
            query.filter = false;//	USE FINAL FILTER BELOW!! -- from ace
        	if (query.filter !== false && word && !(query.caseInsensitive ? prop.toLowerCase() : prop).ContainsIgnoreChars(word)) { return; } //- BEFORE ADDING SMARTCONTAINS
        ...
        //Gather custom completions from my plugin        //MY CUSTOM CODE (note, my add ons are after sort, so they always go to the bottom)
        if (typeof CodeMirrorCustomCompletions  != "undefined" && CodeMirrorCustomCompletions.length > 0) {//CodeMirrorCustomCompletions is global var in my plugin            
            for (var n = 0; n < CodeMirrorCustomCompletions.length; n++) {
                var name = CodeMirrorCustomCompletions[n].name.toLowerCase();
                //if (name.indexOf(word) !== -1) {
                if (name.ContainsIgnoreChars(word)) {
                    completions.push(CodeMirrorCustomCompletions[n]);
                }
            }
        }

    	//logO(completions, 'completions');
		//FILTER IS OFF ABOVE-- use custom filering from ACE--- formatting.js holds the filtering function
        var fl = new FilteredList(completions);
        fl.setFilter(word);
        var filteredCompletions = fl.filtered;
        completions = filteredCompletions;
    	// logO(filteredCompletions, 'filteredCompletions');
    	//end of my code    
