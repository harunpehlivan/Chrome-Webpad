...
document.addEventListener('DOMContentLoaded', function () {   
   /* var files = ["ecma5.json", "lib/tern/defs/browser.json", "lib/tern/defs/jquery.json"];
    var loaded = 0;
    for (var i = 0; i < files.length; ++i) (function (i) {
        GetFile(files[i], function (json) {
            defs[i] = JSON.parse(json);
            if (++loaded === files.length) startServer('', { requirejs: {}, doc_comment: true }, '');
        });
    })(i);
    //Gets file for loading
    function GetFile(file, c) {
    ...
    //NOTE: problem loading files-- results in No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'null' is therefore not allowed access. This is likely due to testing on local machine as chrome wont allow loading files from local machine... but I'm not completely sure, so in the mean time quick fix is to put contents of files in this file 
    var defs = [];
    //wrapped so it collapses in visual studio
    (function () {       
        defs.push({
            "!name": "browser",
            "location": {
    ...
    startServer(defs, { requirejs: {}, doc_comment: true }, '');
